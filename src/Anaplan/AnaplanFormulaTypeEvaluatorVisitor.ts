...
// This class is used to evaluate the formula text.
// It is used by the AnaplanFormulaEvaluator class.

// in this part we used the visitor pattern to visit the parse tree and evaluate the formula text
...
//by default all the expression will be evaluated as unknown
...
//visior for the expressions in general
  visitFormula(ctx: FormulaContext): Format {
    this.formulaErrors.length = 0;
    return this.visit(ctx.expression());
    
  }
//visitor to evaluate expressions in parenthesis
...
//visitor to evaluate the if expression
...
//visitor to evaluate the expressions in binary operations
...
//if the left expression is not a boolean type, an error will be produced
...
//if the right expression is not a boolean type, an error will be produced
...
//visitor to evaluate the expressions in multiplication and division operations
  visitMuldivExp(ctx: MuldivExpContext): Format {
    let leftResult = this.visit(ctx._left);
    let rightResult = this.visit(ctx._right);
//if the left expression is not a number type, an error will be produced
...
//if the right expression is not a number type, an error will be produced
...
//visitor to verify the expressions type in addition and subtraction operations
...
//visitor to evaluate expression type in comparison operations
  visitComparisonExp(ctx: ComparisonExpContext): Format {
    let leftResult = this.visit(ctx._left);
    let rightResult = this.visit(ctx._right);
//if the right and left expressions are not of the same type, an error will be produced
...
//visitor to evaluates expressions type that will be concatenated
  visitConcatenateExp(ctx: ConcatenateExpContext): Format {
    let leftResult = this.visit(ctx._left);
    let rightResult = this.visit(ctx._right);
//if the left expression in not a txt type, an error will be produced
    if (leftResult.dataType != AnaplanDataTypeStrings.TEXT.dataType) {
...
//if the right expression in not a txt type, an error will be produced
...
//visitor to evaluate the expression type in the logical operations(negqtion)
  visitNotExp(ctx: NotExpContext): Format {
    ...
    if (format.dataType != AnaplanDataTypeStrings.BOOLEAN.dataType) {
      //if the expression is not q boolean type, an error will be produced
...
//visitor to evaluates string literals
...
//visitor to check if a function has its parameters in the correct format
...
//visitor to check for the brackets in the formula
